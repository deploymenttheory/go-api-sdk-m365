...
func main() {
	...
	updateRequest := &intuneSDK.ResponseRemoteAssistanceSettings{
		ID:                               "remoteAssistanceSettings",
	...
	// Update settings
	remoteHelpSettings, err := intune.UpdateRemoteHelpSettings(updateRequest)
	if err != nil {
		log.Fatalf("Failed to update remote help settings: %v", err)
	}

	// Pretty print the device management scripts
	jsonData, err := json.MarshalIndent(remoteHelpSettings, "", "  ")
	if err != nil {
