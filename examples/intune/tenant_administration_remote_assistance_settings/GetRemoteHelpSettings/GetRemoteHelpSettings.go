...
func main() {
	...
	remoteHelpSettings, err := intune.GetRemoteHelpSettings()
	if err != nil {
		log.Fatalf("Failed to get remote help settings: %v", err)
	}

	// Pretty print the result
	jsonData, err := json.MarshalIndent(remoteHelpSettings, "", "  ")
	if err != nil {
		log.Fatalf("Failed to marshal remote help settings: %v", err)
	}
