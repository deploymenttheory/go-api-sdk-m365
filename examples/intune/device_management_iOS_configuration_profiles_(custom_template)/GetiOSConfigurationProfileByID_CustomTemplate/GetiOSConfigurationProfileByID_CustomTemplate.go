...
func main() {
	...
	iOSConfigurationProfileID := "480e8223-4f70-437c-9d3c-19b2897404da"

	// Use the Intune client to perform operations
	iOSConfigurationProfile, err := intune.GetiOSConfigurationProfileByID_CustomTemplate(iOSConfigurationProfileID)
	if err != nil {
		log.Fatalf("Failed to get device management scripts: %v", err)
	}

	// Pretty print the device management scripts
	jsonData, err := json.MarshalIndent(iOSConfigurationProfile, "", "  ")
	if err != nil {
	...
	decodedContent, err := utils.Base64Decode(iOSConfigurationProfile.Payload)
	if err != nil {
