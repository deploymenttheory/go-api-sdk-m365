...
func Backup(client *intuneSDK.Client, outputPath, outputFormat string, excludeAssignments bool, prefix string, appendID bool) error {
	log.Println("Starting compliance script backup...")

	// Retrieve all Device Management Scripts
	scripts, err := client.GetDeviceComplianceScripts()
	if err != nil {
		log.Println("Error getting compliance scripts:", err)
		return err
	}

	log.Printf("Found %d compliance scripts\n", len(scripts.Value))
	...
	for _, script := range scripts.Value {
		...
		detailedScript, err := client.GetDeviceComplianceScriptByID(script.ID)
		if err != nil {
	...
	log.Println("Device compliance script backup completed.")
	return nil
}
